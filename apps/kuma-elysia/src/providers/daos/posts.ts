...
export class PostsDaoRls implements IPostDao {
  private rlsConn: ReturnType<RlsClient['rls']>;
  constructor(token: SupabaseToken) {
    const rlsClient = createRlsClient();
    this.rlsConn = rlsClient.rls(token);
  }

  public async getPosts(query: GetPostsQuery) {
    const { languageCode, limit, cursor, reverse } = query;
    const posts = await this.rlsConn(async (tx) =>
      tx
        .select({
          id: postsTable.id,
          updatedAt: postsTable.updatedAt,
        ...
        .innerJoin(
          postsContentTable,
          eq(postsTable.id, postsContentTable.postId),
        )
        .where(eq(postsContentTable.languageCode, languageCode))
        .limit(limit + 1)
        .offset(cursor ?? 0)
  ...
  public async getTagsByPostId(id: number) {
    const tags = await this.rlsConn(async (tx) =>
      tx
        .select({
          value: tagsTable.value,
        })
  ...
  public async getMoments(query: GetMomentsQuery) {
    const { limit, cursor, reverse } = query;
    const moments = await this.rlsConn(async (tx) =>
      tx
        .select({
          id: momentsTable.id,
          updatedAt: momentsTable.updatedAt,
